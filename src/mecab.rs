...
impl Tagger {
    ...
    pub fn parse_str<T: Into<Vec<u8>>>(&self, input: T) -> String {
        unsafe { ptr_to_string(mecab_sparse_tostr(self.inner, str_to_ptr(input))) }
    }
    ...
    pub fn parse_nbest<T: Into<Vec<u8>>>(&self, n: usize, input: T) -> String {
        unsafe { ptr_to_string(mecab_nbest_sparse_tostr(self.inner, n, str_to_ptr(input))) }
    }
...
impl Lattice {
    ...
    pub fn set_result<T: Into<Vec<u8>>>(&self, result: T) {
        unsafe { mecab_lattice_set_result(self.inner, str_to_ptr(result)) }
    }
...
impl Model {
    pub fn new(args: &str) -> Model {
        unsafe { Model { inner: mecab_model_new2(str_to_ptr(args)) } }
    }
...
fn ptr_to_string(ptr: *const c_char) -> String {
    unsafe { str::from_utf8_unchecked(CStr::from_ptr(ptr).to_bytes()).to_owned() }
}
